...
def load_data():
    with get_conn() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            cur.execute("SELECT id, winner, loser, type FROM games ORDER BY id")
            games = [{"id": row["id"], "winner": row["winner"], "loser": row["loser"], "type": row["type"]} for row in cur.fetchall()]
    ...
    def do_POST(self):
        ...
        if path == "/api/teams":
            ...
            with get_conn() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        "INSERT INTO games (winner, loser, type) VALUES (%s, %s, %s)",
                        (winner, loser, result_type)
                    )
                conn.commit()
            self.send_json({"ok": True, **load_data()})
